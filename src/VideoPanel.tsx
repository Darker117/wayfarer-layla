...
import { videoId } from './comfyWorkflow';
import { checkComfy, submitComfy, listComfy, cancelComfy, removeComfy, forgetUnconfirmed, uploadComfy, ComfyRejection } from './comfy';
import { Field, Modal } from './ui';
import { VideoClip } from './VideoClip';
import { connectionUrl, prepareVideoPrompt, publicVideoText, validateVideoDuration, readVideoConnection, readVideoPreferences, saveVideoConnection, saveVideoPreferences, videoPrompt, type VideoJob, type VideoPreferences } from './video';
...
export function useVideo(adventure: Adventure, draft: string, storyBusy: boolean, onSubmitted: (input: string) => void) {
  ...
  const [desktopUrl,setDesktopUrl] = useState(connection?.url || '');
  ...
  const pendingKey = 'wayfarer-comfy-pending:' + adventure.id;
  ...
  async function refresh(c = connection) {
    if (!c) return;
    const rows = await listComfy(c, adventure.id);
    const p = pendingRef.current;
    if (p && !lock.current && rows.some(j=>j.id===p.body.id && j.submitted)) {
      if (p.cancel) await cancelComfy(c,p.body.id,adventure.id);
      else if (typeof p.input === 'string') onSubmitted(p.input);
      persistPending(null); setError('');
    }
    if (alive.current) { setJobs(rows.sort((a,b) => b.createdAt - a.createdAt)); setConnectionStatus('Video records loaded · direct ComfyUI connection'); }
  }
  useEffect(() => {
  ...
  async function start() {
    ...
    if (!connection) { setError('Open Video settings at the bottom left to connect to ComfyUI, then send again. Your draft is still in the composer.'); return; }
    ...
    try {
      ...
      const system = await checkComfy(connection,controller.signal);
      if (prefs.imageId && prefs.imageServer !== connection.url) throw new Error('Upload the starting image again for this PC connection.');
      ...
      if(duration>15 && Array.isArray(system.system.argv) && !system.system.argv.some((arg:string)=>['--cache-none','--cache-ram'].includes(arg))) throw new Error('For long videos, start ComfyUI with --cache-none to release intermediate frames. See Connection setup in Video settings.');
      ...
      persistPending({body,input:draft,cancel:false,desktopUrl:connection.url});
      const job = await submitComfy(connection,body);
      if (cancelled.current) await cancelComfy(connection,job.id,adventure.id);
      ...
      if (e instanceof ComfyRejection) persistPending(null);
  ...
  async function recoverPending() {
    ...
    await action(async()=>{
      const p=pendingRef.current;
      if (p.desktopUrl !== connection.url) throw new Error('Reconnect the PC used for this pending submission.');
      const rows = await listComfy(connection,adventure.id), job = rows.find(j=>j.id===p.body.id);
      if (!job?.submitted) throw new Error('ComfyUI has not confirmed this submission. Check its queue before clearing the record. Recovery never starts a second render.');
      if (p.cancel) await cancelComfy(connection,p.body.id,adventure.id);
      else if (typeof p.input === 'string') onSubmitted(p.input);
  ...
  async function cancelPreparation() {
    ...
    const p=pendingRef.current;
    if (p && connection) { persistPending({...p,cancel:true}); await action(async()=>{const job=await cancelComfy(connection,p.body.id,adventure.id);if(job.state==='cancelled')persistPending(null);await refresh();}); }
  }
  async function connect() {
    await action(async () => {
      if (lock.current || uploading) throw new Error('Wait for video preparation or image upload before changing the PC connection.');
      const next = { url:connectionUrl(desktopUrl) };
      await checkComfy(next); saveVideoConnection(next); setConnection(next); setJobs([]); setConnectionStatus('Connected directly to ComfyUI');
    });
  }
  async function upload(file?: File) {
    if (!file) return;
    if (!connection) { setError('Connect to ComfyUI before uploading a starting image.'); return; }
    setUploading(true);
    await action(async () => {
      const imageId = await uploadComfy(connection,file);
      changePrefs({ imageId,imageName:file.name,imageServer:connection.url });
    }); setUploading(false);
  }
  async function clipAction(job: VideoJob, operation: 'cancel' | 'remove' | 'forget') {
    if (!connection) throw new Error('Reconnect to ComfyUI to manage this clip.');
    if (operation === 'cancel') await cancelComfy(connection,job.id,adventure.id);
    else if (operation === 'forget') {
      await forgetUnconfirmed(connection,job.id,adventure.id);
      if(pendingRef.current?.body?.id===job.id)persistPending(null);
    }
    else {
      await removeComfy(connection,job.id,adventure.id);
  ...
  async function clearPending() {
    await action(async()=>{
      if(!connection || !pendingRef.current)return;
      await forgetUnconfirmed(connection,pendingRef.current.body.id,adventure.id);
      persistPending(null);await refresh();
    });
  }
  ...
  const notices = <>
    ...
    {pending && !phase && <div className="notice">A previous submission needs recovery. Clearing its record does not cancel a request that arrives later; check ComfyUI first.<div className="toolbar"><button className="button" onClick={()=>void recoverPending()}>Recover submission</button><button className="button" onClick={()=>void cancelPreparation()}>Cancel pending submission</button><button className="text-button" onClick={()=>void clearPending()}>Clear unconfirmed record</button></div></div>}
  ...
  return {
    ...
    panel: open && <Modal title="Video settings" wide onClose={() => setOpen(false)}><div className="form-body video-studio">
      ...
      <details open={!connection}><summary>PC connection</summary><p className="subtle">Run ComfyUI on your PC and enter its address. For access away from home, use your private Tailscale HTTPS address.</p><Field label="ComfyUI address"><input type="url" value={desktopUrl} placeholder="https://your-desktop.your-tailnet.ts.net" onChange={e=>setDesktopUrl(e.target.value)}/></Field><div className="toolbar"><button className="button" onClick={()=>void connect()}>Connect to ComfyUI</button>{connection && <><button className="button" onClick={()=>void action(async()=>{await checkComfy(connection);setConnectionStatus('Connected directly to ComfyUI');})}>Test connection</button><button className="text-button" onClick={()=>void action(async()=>{if(lock.current || uploading)throw new Error('Wait for video preparation or image upload before disconnecting.');saveVideoConnection(null);setConnection(null);setJobs([]);setConnectionStatus('Disconnected');})}>Disconnect</button></>}</div><p className="subtle">Disconnecting leaves accepted renders running on the PC. Reconnect to the same address to manage them.</p><p className="subtle" role="status">{connectionStatus}</p><p className="subtle">ComfyUI needs cross-origin access enabled for Layla. <a href="https://github.com/Darker117/wayfarer-layla/blob/main/docs/VIDEO.md" target="_blank" rel="noreferrer">Connection setup</a></p>{localStorage.getItem('wayfarer-video-v1:connection') && <p className="subtle">Clips from the earlier companion remain on your PC. This connection tracks new ComfyUI videos; it does not move or delete your earlier files.</p>}</details>
      <section className="video-clips"><h3>Adventure videos</h3><p className="subtle">View, save, share, or remove videos from this adventure. Removing a clip clears its story entry and ComfyUI history; its file remains in the PC’s ComfyUI output/wayfarer folder.</p>{!jobs.length && <p className="subtle">Your videos will appear here. Closing settings does not stop a render.</p>}{connection && jobs.map(job => <VideoClip view="manager" key={job.id} job={job} input={messages[job.id]?.input} connection={connection} onAction={clipAction}/>)}</section>
    </div></Modal>,
  };
}
