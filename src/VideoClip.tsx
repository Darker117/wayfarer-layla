...
export function VideoClip({ job, input, connection, onAction, suspended = false }: {
  ...
  return <article ref={container} className="story-turn video-response" aria-label="Video response" data-video-id={job.id}>
    <div className="player-action"><span><Film size={16}/></span><p><strong className="action-kind">Video</strong>{input || 'A video of your story'}</p></div>
    <div className="video-clip">
      <div className="video-clip-heading"><strong>{job.resolution ? `${job.resolution} · ${job.duration}s · 16:9` : 'Video request'}</strong><span className="eyebrow">{job.state}</span></div>
      ...
      {job.state === 'completed' && <div className="video-screen">{media
        ? <video aria-label="Generated adventure video" controls playsInline preload="metadata" src={media.url}/>
