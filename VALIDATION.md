...
## Version 1.5.0 — Direct ComfyUI connection

- The mini-app connects directly to native ComfyUI APIs. The PC Companion, pairing codes, MCP process and gateway are no longer required or packaged. Existing companion state and files remain intact. The legacy source remains for older installations.
- **82 core tests and 43 browser checks pass**, including all 1–225-second allocations, exact native graph links, optional starting images, all enhancement/AI-duration combinations, UUID persistence, uncertain-response recovery without resubmission, scoped cancellation, record clearing after a PC restart, disconnect/reconnect, actual fixture playback, settings-only management and mobile layouts. The final full browser run passed all 43 checks. Connection tests wait on persisted state rather than transient messages.
- **Real direct model smoke:** ComfyUI 0.36.0 accepted the production Hermes/FastH3 graph through `/prompt` and produced a neutral five-second clip. Final video: H.264, 854×480, 120 frames at 24 fps, exactly 5.000 seconds; AAC audio: exactly 5.000 seconds. Observed completion was about 85 seconds on this setup, including model preparation. One sampled frame was inspected. This is a smoke test, not a general speed/quality benchmark.
- **Native 225-second synthetic smoke:** fifteen segments, fourteen last-frame extraction/continuation links, native trimming, encoding and concatenation completed in one ComfyUI job. Output had 5,400 frames and exactly 225.000 seconds of video and audio. Synthetic 64×64 frames avoided model calls; this does not establish 225-second AI generation speed or visual continuity. Reproduce with `npm run test:comfy-direct`.
- Private Tailscale HTTPS was pointed directly at ComfyUI. `/system_stats` returned HTTP 200 with the expected local-file CORS origin; MP4 range retrieval returned HTTP 206 and the requested 1,024 bytes. The idle legacy companion was stopped, with its completed clips and state preserved. ComfyUI launch options for local-file CORS and intermediate-cache release were saved in Stability Matrix, preserving other packages and flags.
- Playback now uses native media range requests instead of loading entire MP4s into JavaScript memory. Save/share downloads happen only on request. Removing a clip removes its story entry and ComfyUI history, **not its output file**; this distinction is visible in settings and documented. Restarted/failed renders do not silently regenerate; direct mode has no segment-resume/export-retry feature.
- **Remaining user checks:** install this release in Layla and verify the direct WebView connection, uploaded starting images, video/audio playback and Android save/share. No phone interaction or personal-story generation was performed for this release. Story saves/cards and prior script/streaming behavior remain covered by the regression suite.
